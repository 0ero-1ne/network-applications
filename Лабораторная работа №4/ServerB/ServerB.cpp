...
void main()
{
	try
	{
		...
		while (true)
		{
			...
			std::cout << endl << "IP: " << inet_ntoa(from.sin_addr) << "\tport: " << htons(from.sin_port);
			hostent* clientHost = gethostbyaddr(inet_ntoa(from.sin_addr), sizeof(from.sin_addr), AF_INET);
			if (clientHost == NULL) cout << "\tNULL";
			lbfrom = sizeof(from);
