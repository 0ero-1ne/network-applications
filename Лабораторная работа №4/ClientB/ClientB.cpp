...
//‘ункци€ предназначена дл€ отправки широковещательного запроса в локальную сеть 
bool GetServer(
...
)
{
	...
	int optval = 1;																				// состо€ние enabled
	//установить опции сокета дл€ использовани€ широковещательного адресса
	if (setsockopt(clientSocket,
		SOL_SOCKET,																				// уровень д-ви€ режима
		SO_BROADCAST,																			// режим сокета дл€ исп-ни€ широковещ.адреса
	...
	std::cout << std::endl << "ѕорт сервера: " << addr->sin_port;
	std::cout << std::endl << "IP-адрес сервера: " << inet_ntoa(addr->sin_addr);

	if (!strcmp(nameServer, call))
	{
		std::cout << std::endl << "—ервер с таким именем найден.";
		return true;
	}
	else
	{
		std::cout << std::endl << "—ервер с таким именем не найден.";
		return false;
	}
